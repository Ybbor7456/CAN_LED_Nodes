...
#include <Wire.h>                 //TWI library wire.begin(), wire.Setclock()
...
// constexpr uint8_t SENS_PIN = 8;   // PIR OUT pin -> removed and replaced with potentiometer
...
constexpr int DRAW_H = 20;
constexpr int DRAW_W = 88;
constexpr int8_t OLED_RESET = -1;           // No reset pin on many I2C module
constexpr uint8_t OLED_ADDR = 0x3C;

Adafruit_SSD1306 display(SCREEN_WIDTH, SCREEN_HEIGHT, &Wire, OLED_RESET); // for display(); 
...
static uint16_t avg_adc(uint8_t n = 8, int pin = POT_PIN) {
  ...
  for (uint8_t i = 0; i < n; ++i) {
    sum += analogRead(pin);        // 0 - 40955
  }
  return (uint16_t)(sum / n);
}

static int band_for(uint16_t raw) {
  if (raw <=  100)  return -1; // 0-100 off, 101-1400node1, 1401-2800 ode2, 2801-4095 node 3
...
void setup() {
  ...
  Wire.begin(); 
  Wire.setClock(400000);               // faster I^2C 
...
void loop() {
  ...
  if (target != lastTarget) {
    const unsigned ids[3] = { ID_NODE1, ID_NODE2, ID_NODE3 };  // 
    ...
    digitalWrite(LED_PIN, (target == 0) ? HIGH : LOW); 
